...
<!-- 목록 화면 컨트롤	-->
	<form class="" name="preset">
		...
		<div class="preSrch">
			<div class="left">
			기간: [ ${SRCH_V[0] } ~ ${SRCH_V[1] } ] / 내용 : [ ${SRCH_V[2] } ] / 카테고리 : [ ${SRCH_V[3] } ] 검색 결과입니다.
			<input type="button" value="전체보기" onClick="location.href='listTransaction.do?INEX=IN&SLC=${param.SLC }'"></div>
		</div>	
		</c:if>
	</form>
	<div class="set_listCount">
		...
		<div class="right">
			...
			<div id="slc"><form action="listTransaction.do?INEX=IN" name="fmLC">
				<input type="hidden" name="INEX" value="IN">
			<c:if test="${param.SRCH == 'true' }">
...
<%-- 				<input type="hidden" name="METH" value="${SRCH_V[4] }"> --%>
	...
	<form action="listTransaction.do?INEX=IN" name="fmSRCH" >
...
<!-- 실제 목록 -->
	...
	<div class="oneTrans">
		...
		<div class="lower">
			...
			<div class="inner meth hidden">결제수단: ${trans.meth_name }</div>
		</div>
	</div> <!-- oneTrans 끝 -->
	</c:forEach>
	</div> <!-- list 끝 -->
	<div class="page">
		<c:if test="${ param.SRCH == 'false' }">
			...
			<a href="listTransaction.do?INEX=IN&SLC=${param.SLC }&PG=${page }">${page }</a>	
			</c:forEach>
		</c:if>
		<c:if test="${ param.SRCH == 'true' }">
			<c:forEach begin="1" end="${PAGES }" var="page">
			<a href="listTransaction.do?INEX=IN&SLC=${param.SLC }&PG=${page }&
			D_FROM=${SRCH_V[0] }&D_TO=${SRCH_V[1] }&ITEM=${SRCH_V[2] }&CATE=${SRCH_V[3] }">${page }</a>	
			</c:forEach>
		</c:if>
	</div>
</div> <!-- contMain 끝 -->
